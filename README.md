...
Artifacts created in `models/v<MODEL_VERSION>/` (version comes from `MODEL_VERSION`):
...
* Holdout reservation
* Class weighting
* Length bucket augmentation
* Department leakage exclusion via regex
* Engineered priority feature tokens (experimental; currently disabled by default due to no measurable uplift)
* Persisted split indices (`train_indices.txt`, `val_indices.txt`) for reproducible evaluation
```
python train.py \
  ...
  [--priority-extra]  # optional engineered tokens (under evaluation)
```
Flags:
...
- `--department-exclude-regex REGEX` (repeatable): Remove matching token patterns from descriptions ONLY for the department model (prevents leakage from enrichment tokens).
- `--priority-extra`: Adds engineered keyword / structural tokens for priority (current experiments show no improvement; disabled in best config below).

Artifacts now include:
* `holdout_metrics.json` when a holdout split is used.
* `train_indices.txt` / `val_indices.txt` for deterministic re-evaluation.
...
## Model Performance & Experiment History

| Version | Config Highlights | Priority Macro F1 (Val) | Dept Macro F1 (Val) | Priority Macro F1 (Holdout) | Dept Macro F1 (Holdout) | Notes |
|---------|------------------|-------------------------|---------------------|-----------------------------|-------------------------|-------|
...
`scripts/algorithm_comparison.py` explores multiple algorithms & hyperparameters (LogReg C∈{0.5,1,2,5}, LinearSVC C∈{0.5,1,2}, MultinomialNB α∈{0.5,1,2}) with / without engineered priority tokens.

Best results (validation):
* Priority: LogisticRegression C=5.0 (balanced), WITHOUT `--priority-extra` → Macro F1 0.2654
...
python scripts/algorithm_comparison.py \ 
  ...
  --output-dir reports
```
Outputs: `reports/algo_comparison_v1.0.4.{json,md}` sorted by priority macro F1.

### Error Analysis
`scripts/error_analysis.py` loads the persisted validation indices (if present) for consistent comparisons and produces:
* `error_analysis_v<version>.json`
* `error_analysis_v<version>.md` (per-class metrics + high-confidence misclassifications)

Usage:
```powershell
python scripts/error_analysis.py --version 1.0.4 --data data/enriched_customer_tickets.csv --models-dir models --output-dir reports
```

### Next Planned Improvements
* Probability calibration (Brier score + reliability plot)
* Macro-F1 CI gating thresholds
* Removal or refinement of low-signal engineered priority tokens
* Per-target hyperparameters baked into a refactored classifier config
